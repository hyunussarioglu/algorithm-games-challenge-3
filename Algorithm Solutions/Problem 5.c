/* 
	Bu program girilen n içim n. fibonacci sayýsýný ekrana yazdýrýr.(Sayma sayýlarý 1'den baþladýðýndan n'in 0'dan büyük girildiði varsayýlýr)
	|1 1|   matrisinin n-1. kuvveti alýnýp |1|  vektörü ile çarpýldýðýnda oluþan vektöün ikinci elemaný n. fibonacci sayýsýný verir.
	|1 0|                                |0|
*/ 
...
unsigned long long **dot(unsigned long long**, int, int, unsigned long long**, int, int); // Matris çarpýmý yapan fonksiyon
...
int main(void)
{
	...
	result = dot(exp_n(n-1), 2, 2, fibonacci, 2, 1); // Sonuç vektörü hesaplanýr
...
unsigned long long **exp_n(int n)
{
	...
	// Kuvvetini aldýðýmýz matris ve birim matris oluþturulur.
	for (i = 0; i < 2; i++)
	{
	...
	/*
		n>0 olduðu sürece;
		n tek ise matris^n = matris^(n/2) * matris^(n/2) * matris þeklinde hesaplanýr
		n çift ise matris^n = matris^(n/2) * matris^(n/2) þeklinde hesaplanýr
